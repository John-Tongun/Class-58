...
In this exercise, you will simulate a basic server response system using Node.js and the switch statement.
...
Status Code | Meaning 200 | Request successful 400 | Bad request 401 | Unauthorized access 404 | Resource not found 500 | Internal server error

Your program must use a switch statement to determine which message should be displayed.

If the statusCode does not match any known code, the program must display a default message indicating an unknown status code.
...
statusCode = 200 Output: Request successful.

statusCode = 500 Output: Internal server error.

statusCode = 123 Output: Unknown status code.
